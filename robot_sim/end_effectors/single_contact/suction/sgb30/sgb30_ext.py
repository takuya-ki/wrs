...
import robot_sim.end_effectors.single_contact.single_contact_interface_ext as si


class SGB30(si.SingleContactInterface):
    ...
    def __init__(
        ...
        # suction center)
        # self.suction_center_pos = np.array([0, 0, .09])  # 0.8+0.1
        self.suction_center_pos = np.array([0., 0., 0.])  # 0.8+0.1 TODO
        # collision detection
        self.all_cdelements = []
    ...
    def gen_stickmodel(
        ...
        if toggle_tcpcs:
            singlecontact_center_gl_pos = \
                self.rotmat.dot(self.singlecontact_center_pos) + self.pos
            singlecontact_center_gl_rotmat = \
                self.rotmat.dot(self.singlecontact_center_rotmat)
            gm.gen_dashstick(
                spos=self.pos,
                epos=singlecontact_center_gl_pos,
                thickness=.0062,
                rgba=[.5, 0, 1, 1],
                type="round").attach_to(mm_collection)
            gm.gen_mycframe(
                pos=singlecontact_center_gl_pos,
                rotmat=singlecontact_center_gl_rotmat).attach_to(mm_collection)
...
if __name__ == '__main__':
    ...
    grpr.gen_meshmodel(toggle_tcpcs=False).attach_to(base)
